...
if navi == 'Documentation':
    ...
    if subNav == 'Introduction':
        st.write('This is an implementation of a natural language processing application using five machine learning algorithms () in Predicting if a news posts is Peace or War related. The application is built on python. The machine learning models were built with the Scikit-Learn Machine Learning Library and the visualization is built with Streamlit.')
    if subNav == 'Pipeline':
        st.header('Natural Language Processing')
        st.subheader('Raw text')
        st.write('Involves gathering raw text data of news posts from various global news media e.g BBC, Aljezeera, CNN...')
        ...
        st.write('Using the regular expression module from python, news posts will be divided into a list of comma separated words. These words are called tokens')
        st.code('import re')
        ...
        st.write('TF-IDF are word frequency scores that try to highlight words that are more interesting, e.g. frequent words but not across the news posts. The TfidfVectorizer will tokenize words, learn the vocabulary and inverse words frequency weightings, then encode.')
        st.code('from sklearn.feature_extraction.text import TfidfVectorizer')
        st.code('tfidf  = TfidfVectorizer()')
        st.subheader('Machine Learning')
        st.write('In this process the vectorized news posts will be split into a training and testing set on 70% to 30% ratio with the train_test_split function from sklearn.model_selection module')
